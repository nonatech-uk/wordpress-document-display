<?php
/**
 * GitHub Plugin Updater for Document Display
 *
...
class DocumentDisplay_GitHub_Updater {
     ...
     * @param string $repo GitHub repository (e.g., 'nonatech-uk/wp-document-display')
    ...
    public function __construct($plugin_file, $repo, $current_version) {
        ...
        $this->cache_key = 'document_display_github_update_' . md5($repo);
    ...
    public function plugin_info($result, $action, $args) {
        ...
        return (object) array(
            ...
            'sections' => array(
                'description' => 'Display directory contents as a table using a shortcode. Supports recursive folder navigation, sub-documents, and markdown commentary.',
    ...
    public function add_check_update_link($links) {
        $url = wp_nonce_url(admin_url('plugins.php?document_display_check_update=1'), 'document_display_check_update');
    ...
    public function handle_check_update() {
        if (empty($_GET['document_display_check_update'])) {
            return;
        }

        if (!current_user_can('update_plugins') || !wp_verify_nonce($_GET['_wpnonce'], 'document_display_check_update')) {
            return;
        }
